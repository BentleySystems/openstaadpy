...
for b in first_beams:
    try:
        start_forces = output.GetMemberEndForces(
            b, 0, load_case_no, local_flag
        )  # start end
...
for bf in beam_forces_summary:
    if "error" in bf:
        ...
        print(
            f" Beam {bf['beam']} Start FX={bf['start']['FX']:.3f} FY={bf['start']['FY']:.3f} FZ={bf['start']['FZ']:.3f} MX={bf['start']['MX']:.3f} MY={bf['start']['MY']:.3f} MZ={bf['start']['MZ']:.3f} | End FX={bf['end']['FX']:.3f} FY={bf['end']['FY']:.3f} FZ={bf['end']['FZ']:.3f} MX={bf['end']['MX']:.3f} MY={bf['end']['MY']:.3f} MZ={bf['end']['MZ']:.3f}"
        )
...
for nid in support_nodes:
    try:
        ...
        print(
            f" Node {nid} RX={reactions[0]:.3f} RY={reactions[1]:.3f} RZ={reactions[2]:.3f} MX={reactions[3]:.3f} MY={reactions[4]:.3f} MZ={reactions[5]:.3f}"
        )
    except Exception as e:
